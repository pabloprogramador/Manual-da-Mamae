...
$.fn.peqRevista = function (configuracao) {
	configuracao = jQuery.extend({
				...
				for(i = 1; i <= folhasQt ; i++){
					...
					//listaZoom[i].disable();
dentroDiv += '<div id="folha'+i+'" class="folhas"><div id="infolha'+i+'" class="infolhas"><div><img style=" position:absolute;" class="folhas_img" src="'+configuracao.padraoSrcMini+i+'.jpg"/><img style="position:absolute;" class="folhas_img" src=""/></div></div></div>';
...
dentroNav += '<div class="folhasMini"><a href="javascript:passaPaginaNav('+i+');"><img  class="folhas_imgMini" src="'+configuracao.padraoSrcMini+i+'.jpg"/></a></div>';
				...
				function mostraFoto(){
					...
					if($("#folha"+atual+" > div > div").children('img:nth-child(2)').attr('src')==""){
						$("#folha"+atual+" > div > div").children('img:nth-child(2)').attr('src',configuracao.padraoSrc+atual+'.jpg')
					}
